...
def main_menu():
    ...
    while True:
        ...
        if ev.type == pygame.KEYDOWN:
            ...
            draw_text(screen, "or [Q] To Quit , [Tab] To Pause", 30, WIDTH/2, (HEIGHT/2)+40)
    ...
    def update(self):
       ...
       # 이준경
        ...
        if keystate[pygame.K_TAB]:
            draw_text(screen, "Pause for 3 seconds", 30, WIDTH/2, HEIGHT/2)
            pygame.display.flip()
            time.sleep(3)
        ...
        if self.rect.left < 0:
            self.rect.left = 0
        if self.rect.bottom >= HEIGHT - 10: ## 비행기 개체가 밑으로 빠지지 않게 한다
            self.rect.bottom = HEIGHT - 10
...
player_img2 = pygame.image.load(path.join(img_dir, 'playerShip1_orange2.png')).convert() # 이준경
player_img3 = pygame.image.load(path.join(img_dir, 'playerShip1_orange3.png')).convert() # 이준경
